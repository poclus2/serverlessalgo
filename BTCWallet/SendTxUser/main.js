...
exports.handler =  (event, context, callback) => {
    ...
    || (receiver == "" || receiver == null) || (password == "" || password == null)
        ...
        connection.query('SELECT `login`, `customers_id` FROM `customers` WHERE `login`=\'' + username + '\';', function (error, results, fields) {
            ...
            if (error){
                ...
                if (results.length == 0){
                    ...
                    pool.query('SELECT `login`, `wallet`.`public_key_hash` FROM `wallet` INNER JOIN customers ON wallet.customer_id = customers.customers_id  WHERE customers.login =\'' + receiver + '\' AND wallettype_id = ' + 1, function (error, results, fields) {
                        // And done with the connection.
                        //connection.release();
                        // Handle error after the release.
                        if (error){
                            ...
                            if (results.length == 0){
                                callback(null, {headercode : 400, message: 'Incorrect receiver username', value : '', error :true});
                            }else if (results[0].public_key_hash == null || results[0].public_key_hash == 'null' ){
                                callback(null, {headercode : 400, message: 'Reciever address not generated', value : '', error :true});
                            }else{
                                receiveraddress = results[0].public_key_hash;
                                SendBTC();
                            }
                        }
                    });
