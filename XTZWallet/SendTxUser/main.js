...
exports.handler =  (event, context, callback) => {
    ...
    if((username == "" || username == null) || (publickey == "" || publickey == null)
        ...
        || (amount == "" || amount == null) || (receiver == "" || receiver == null)
        ...
        connection.query('SELECT `login`, `customers_id` FROM `customers` WHERE `login`=\'' + username + '\';', function (error, results, fields) {
            ...
            if (error){
                ...
                if (results.length == 0){
                    ...
                    connection.query('SELECT `wallet`.`public_key_hash` FROM `wallet` INNER JOIN customers ON wallet.customer_id = customers.customers_id  WHERE customers.login =\'' + receiver + '\' AND wallettype_id = 3', function (error, results, fields) {
                        // And done with the connection.
                        //connection.release();
                        // Handle error after the release.
                        if (error){
                    ...
                    pool.query(req, (err, rows/*,fields*/) => {
                        ...
                        pool.query(req, (err, rows/*,fields*/) => {
                            ...
                            pool.query(req, (err, results/*,fields*/) => {
                                ...
                                todo = [wallettype_id, customers_id, amount, 0, createtime, 0, 0, idwalletsender, receiver, null, receiveraddress, transactionnote, fees];
